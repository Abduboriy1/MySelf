...
<template>
    <GuestLayout>
        <div class="relative overflow-hidden text-stone-900 min-h-screen rounded-3xl p-4 md:p-6 space-y-4 md:space-y-6">


            <!-- === About === -->
            <section id="about" class="mx-auto max-w-6xl px-6 pt-10 pb-10 bg-white/30 dot-pattern rounded-2xl border border-stone-200 backdrop-blur-[2px]">
                <div class="grid items-center gap-8 md:grid-cols-[1.2fr_.8fr]">
                    <div>
                        <p class="inline-flex items-center gap-2 text-xs tracking-widest text-amber-600">
                        ...
                        <div class="mt-6 flex flex-wrap items-center gap-3 text-sm">
                            <a :href="`mailto:${profile.email}`"
                               class="inline-flex items-center gap-2 rounded-full border border-stone-200 bg-stone-100 px-3 py-1 text-stone-700 backdrop-blur hover:bg-stone-200 hover:text-amber-700 hover:border-amber-400/40 transition">
                            ...
                            <span
                                class="inline-flex items-center gap-2 rounded-full border border-stone-200 bg-stone-100 px-3 py-1 text-stone-700">
                        ...
                        <div class="mt-5 flex flex-wrap gap-3">
                            <a v-for="s in profile.social" :key="s.label" :href="s.href"
                               class="group inline-flex items-center gap-2 rounded-xl border border-stone-200 bg-stone-100 px-3 py-2 text-sm text-stone-700 backdrop-blur hover:bg-stone-200 hover:text-amber-700 transition"
                               rel="noopener"
                               target="_blank">
                        ...
                        <div class="mt-7 flex gap-3">
                            <a :href="profile.resumeUrl"
                               class="inline-flex items-center gap-2 rounded-xl border border-stone-200 bg-stone-100 px-4 py-2 text-sm text-stone-700 backdrop-blur hover:bg-stone-200 transition">
                                <Download class="h-4 w-4"/>
                                Download Resume
                            </a>
                            <a :href="`mailto:${profile.email}`"
                               class="inline-flex items-center gap-2 rounded-xl border-0 bg-gradient-to-r from-amber-500 to-orange-500 px-4 py-2 text-sm text-white hover:from-amber-400 hover:to-orange-400 transition">
                                Contact
                    ...
                    <div class="relative">
                        <div class="p-[2px] bg-gradient-to-br from-amber-400/40 to-orange-400/40 rounded-3xl mx-auto w-64 md:w-72">
                            <div class="relative aspect-square overflow-hidden rounded-3xl bg-stone-100 shadow-2xl">
                                <img v-if="me" :src="me" alt="Headshot" class="h-full w-full object-cover"/>
                                <div v-else class="flex h-full w-full items-center justify-center text-stone-600">Add your
                                    photo
                                </div>
                            </div>
                        </div>
                        <div class="absolute -bottom-10 -left-4 flex gap-3 mt-10">
                            <div v-for="h in profile.highlights" :key="h.k"
                                 class="rounded-2xl border border-stone-200 bg-white/80 px-4 py-3 text-center backdrop-blur">
                                <div class="text-xl text-amber-600 font-semibold">{{ h.k }}</div>
            ...
            <!-- === Experience === -->
            <section id="experience" class="relative mx-auto max-w-6xl px-6 py-10 bg-white/30 dot-pattern rounded-2xl border border-stone-200 backdrop-blur-[2px]">

                <!-- header -->
                <header class="relative z-10 flex items-center justify-between gap-3">
                    <div class="flex items-center gap-3">
                        <div class="grid size-10 place-items-center bg-stone-100 rounded-xl text-amber-600">
                            <Briefcase class="size-5"/>
                        </div>
                        <div>
                            <h2 class="text-2xl font-semibold tracking-tight">Experience</h2>
                            <p class="text-sm text-stone-600">Impact, scope, and the tools that got it done.</p>
                        </div>
                    </div>

                    <!-- subtle pill -->
                    <span class="hidden md:inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs
             bg-stone-100 ring-1 ring-stone-200 text-stone-700">
           ...
           bg-gradient-to-b from-transparent via-amber-500/25 to-transparent"></div>
             ...
             bg-gradient-to-br from-amber-400 to-orange-400 ring-2 ring-offset-2 ring-offset-[#fdfaf4] ring-stone-300">
                        </div>
                        ...
                        <div class="pl-10 pr-2 md:pl-0 md:pr-8 md:text-right md:order-1">
                            ...
                            <div class="mt-2 flex items-center justify-end gap-2">
                                <h3 class="text-lg font-semibold leading-snug text-stone-900">{{ job.role }}</h3>
                 ...
                 hover:ring-amber-400/40 hover:text-amber-700
               ...
               transition-all duration-300
               hover:shadow-xl hover:shadow-amber-500/10 hover:ring-amber-400/30">

                                <div class="relative p-6">
                                    <ul class="space-y-2.5 text-stone-600">
                                        <li v-for="(b, i) in job.bullets" :key="i" class="flex gap-3">
                                            <span class="mt-2.5 size-1.5 shrink-0 rounded-full bg-amber-400/70"></span>
            ...
            <!-- === Skills === -->
            <section id="skills" class="mx-auto max-w-6xl px-6 py-10 bg-white/30 dot-pattern rounded-2xl border border-stone-200 backdrop-blur-[2px]">
                <h2 class="flex items-center gap-2 text-2xl font-semibold">
                    <Code2 class="h-6 w-6 text-amber-600"/>
                    Skills
                </h2>
                <div class="mt-5 flex flex-wrap gap-2">
                    <span v-for="s in profile.skills" :key="s"
                          class="rounded-full border border-stone-200 bg-stone-100 px-3 py-1 text-sm text-stone-700 hover:bg-stone-200 hover:border-amber-400/40 hover:text-amber-700 transition cursor-default">{{
                            s
                        }}</span>
                </div>
            </section>
            ...
            <!-- === Certifications === -->
            <section id="certifications" class="mx-auto max-w-6xl px-6 py-10 bg-white/30 dot-pattern rounded-2xl border border-stone-200 backdrop-blur-[2px]">
                <h2 class="flex items-center gap-2 text-2xl font-semibold">
                ...
                <ul class="mt-6 grid gap-4 sm:grid-cols-2">
                    <li v-for="(c, i) in profile.certifications" :key="i"
                        class="rounded-xl border border-stone-200 bg-white/70 p-4 backdrop-blur">
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <span
                                class="shrink-0 rounded-full border border-stone-300 bg-stone-100 px-2 py-0.5 text-xs text-amber-700">
                                {{ c.year }}
                            </span>
                        </div>
            ...
            <!-- === Contact === -->
            <section id="contact" class="mx-auto max-w-6xl px-6 py-10 bg-white/30 dot-pattern rounded-2xl border border-stone-200 backdrop-blur-[2px]">
                <h2 class="flex items-center gap-2 text-2xl font-semibold">
                    <Mail class="h-6 w-6 text-amber-600"/>
                    Contact
                </h2>
                <p class="mt-2 text-stone-600">Get in touch — I'm open to new opportunities.</p>
                <div class="mt-6 flex flex-wrap gap-3 text-sm">
                    <a :href="`mailto:${profile.email}`"
                       class="inline-flex items-center gap-2 rounded-full border border-stone-200 bg-stone-100 px-3 py-1 text-stone-700 hover:bg-stone-200 hover:text-amber-700 hover:border-amber-400/40 transition">
         ...
         bg-gradient-to-r from-amber-500 to-orange-500 px-4 py-2 text-sm font-medium text-white shadow-xl shadow-amber-500/20
         backdrop-blur hover:from-amber-400 hover:to-orange-400 active:scale-[0.98] transition
           ...
           bg-white" role="dialog">
                    ...
                    <header
                        ...
                        <button aria-label="Close"
                                class="rounded-lg border border-stone-200 bg-stone-100 p-2 text-stone-700 hover:bg-stone-200"
